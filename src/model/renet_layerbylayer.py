...
class ReNet_lightning_finetune(pl.LightningModule):
    def __init__(self):
        super().__init__()
    ...
    def add_layer(self,layer,size):
         self.features.append(layer)
         self.features.append(nn.Dropout(0.3))
         self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(320 * size * size, 4096),
            nn.ReLU(),
            nn.Linear(4096, 10),
            )
    ...
    def configure_optimizers(self):
        self.optimizer = torch.optim.Adam(self.parameters(), lr=0.00001, weight_decay =3e-4)
...
class MLP_renet_layerbylayer(pl.LightningModule):
